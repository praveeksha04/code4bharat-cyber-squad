...
if (!AZURE_STORAGE_CONNECTION_STRING || !SPEECH_KEY || !SPEECH_REGION) {
...
function getSharedKeyCredential() {
  ...
  if (!accountName || !accountKey) {
    throw new Error(
      "Could not parse AccountName/AccountKey from Connection String"
    );
  }
...
async function pollForResult(statusUrl) {
  const headers = { "Ocp-Apim-Subscription-Key": SPEECH_KEY };
  while (true) {
    ...
    if (status === "Succeeded") {
      ...
      const resultFile = resultResponse.data.values.find(
      ...
      throw new Error(
        `Transcription failed: ${
          response.data.properties?.error?.message || "Unknown error"
        }`
      );
    }
  }
...
router.post("/", upload.single("video"), async (req, res) => {
  ...
  try {
    ...
    const speechToTextEndpoint = `https://${SPEECH_REGION}.api.cognitive.microsoft.com/speechtotext/v3.1/transcriptions`;
    ...
    if (fs.existsSync(originalUploadedPath))
